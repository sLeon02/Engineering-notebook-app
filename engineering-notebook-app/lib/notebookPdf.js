...
function drawPageShell(pdfDoc, fonts, { project, pageNumber, continuation, signature }) {
  ...
  // signature block — present on every page. If the entry has been signed off
  // on the site, the name/date print in ink above the line; otherwise the
  // line stays blank for a physical signature later.
  const sig = signature || {};
  const sigY = 50;

  if (sig.enteredBy) {
    page.drawText(sig.enteredBy, { x: MARGIN_LEFT + 2, y: sigY + 17, size: 9.5, font: fonts.mono, color: INK });
  }
  ...
  if (sig.enteredOn) {
    page.drawText(sig.enteredOn, { x: 322, y: sigY + 17, size: 9.5, font: fonts.mono, color: INK });
  }
  ...
  if (sig.witnessBy) {
    page.drawText(sig.witnessBy, { x: MARGIN_LEFT + 2, y: sigY - 17, size: 9.5, font: fonts.mono, color: INK });
  }
  ...
  if (sig.witnessOn) {
    page.drawText(sig.witnessOn, { x: 322, y: sigY - 17, size: 9.5, font: fonts.mono, color: INK });
  }
...
async function buildNotebookPdf(pdfDoc, { project, entries }, fetchPhoto) {
  ...
  function newPage(continuation, signature) {
    pageNumber += 1;
    page = drawPageShell(pdfDoc, fonts, { project, pageNumber, continuation, signature });
    cursorY = TOP_START;
  }

  function ensureSpace(height, signature) {
    if (cursorY - height < BOTTOM_RESERVED) {
      newPage(true, signature);
    }
  }

  // cover page (no signature block filled in — it's not a dated entry)
  newPage(false, null);
  ...
  for (const entry of ordered) {
    const signature = {
      ...
      witnessBy: entry.witness_by || '',
      witnessOn: entry.witness_on || '',
    };

    newPage(false, signature);
    ...
    for (const line of lines) {
      ensureSpace(lineHeight, signature);
